...
export const appApi = {
  ...
  /** 扫描学习资料。 */
  scanStudyNotes: (studyRoot: string) => getBridge().scanStudyNotes({ studyRoot }),
  /** 读取 Markdown 文件内容。 */
  readMdContent: (filePath: string, studyRoot: string) => getBridge().readMdContent({ filePath, studyRoot }),
  /** 获取学习进度映射。 */
  ...
  /** 写入轻量偏好。 */
  setPreference: (key: string, value: string) => getBridge().setPreference(key, value),
  /** 用 VSCode 打开路径。 */
  openInVscode: (targetPath: string) => getBridge().openInVscode(targetPath),
  /** 打开系统目录选择器。 */
  selectDirectory: (payload?: { defaultPath?: string }) => getBridge().selectDirectory(payload),
}
